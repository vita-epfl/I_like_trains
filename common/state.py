...
class RoomChoice(BaseModel, frozen=True):
    # set to True if you want the server to wait for a response. Akin to playing
    # a board game. Set this to False for the game to be "real-time". This will
    # be set to False at grading time.
    slow: bool = False
...
class Map(BaseModel):
    ...
    width: int
    height: int
    available_cells: set[Pos]  # places where buses can go
    delivery_zones: dict[
        Pos, Dest
...
class GameState(BaseModel):
    ...
    passengers: dict[Pos, Passenger] = {}  # passengers waiting to be picked up
