...
class Game:
    ...
    def __init__(self, random: random.Random, room_state: RoomState, room_id: int):
        self.random = random
        ...
        for slot in range(0, self.room_state.room_choice.total_players):
            self.spawn_bus(slot)
    ...
    def bus_is_alive(self, slot: Slot) -> bool:
        """
        Check if this player's bus is alive
        """
        bus = self.game_state.buses[slot]
        if bus.respawn_at > 0:
    ...
    def move(self, slot: Slot, move: Move) -> None:
        try:
            for passenger_id in move.drop:
                self.drop(slot, passenger_id, False)
            ...
            # It's really bad if we get here. That means the game code
            # has a serious logic bug. We continue but we update the
            # status to record the error.
            logger.exception(e)
            logger.error("game is broken")
    ...
    def pickup(self, slot: Slot, passenger_id: PassengerId) -> None:
        ...
        logger.debug(f"#{self.room_id} {slot} picked up passenger {passenger_id}")

    def drop(self, slot: Slot, passenger_id: PassengerId, is_crash: bool) -> None:
        """
        ...
        This method is also called when the bus crashes to first remove all the
        passengers.
        """
        bus = self.game_state.buses[slot]
        passenger = None
        ...
        pos = bus.positions[0]
        if not is_crash:
            # You can't score points if you crash and the passenger happens to be dropped off
            # at the right place
            ...
            if d == passenger.destination:
                ...
                logger.debug(
                    f"#{self.room_id} {slot} dropped {passenger_id} ({passenger.value}) at their destination, new score: {self.game_state.scores[slot]}"
                )
                self.spawn_passenger()
                return

        # Find a spot to put the passenger. If we can't find a spot, we'll spawn
        # a new passenger
    ...
    def crashed(self, slot: Slot, new_x: int, new_y: int):
        logger.debug(f"#{self.room_id} {slot} crashed their bus ({new_x}, {new_y})")
        bus = self.game_state.buses[slot]

        # Remove the passengers
        passengers = list(bus.passengers)
        for passenger in passengers:
            self.drop(slot, passenger.id, True)
